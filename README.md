...
- **16×16 点阵中文字库** — 使用文泉驿点阵宋体 11pt BDF（约 15px），从官方 PCF 点阵字库提取，内置 119 个常用汉字，笔画清晰、无锯齿
...
- 新增菜单:
  - **SetSav**（显示屏保）— 完整汉化（关/LOGO/LOGO+/黑客帝国）
- 新增汉字（字库扩展至 119 个）:
  - **志**（U+5FD7）、**矩**（U+77E9）、**阵**（U+9635）、**黑**（U+9ED1）、**客**（U+5BA2）、**帝**（U+5E1D）、**国**（U+56FD）
...
| **FLASH** | 102,092 B | 118 KB | 84% |

汉化部分仅额外占用少量 FLASH（中文字库 + 渲染代码），RAM 占用与原版完全一致。
